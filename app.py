...
def index():
    ...
    yaml_text = yaml.dump(spec, sort_keys=False)
...
def generate_yaml():
    ...
    for path_item in data.get("paths", []):
        path_url = path_item.get("url", "").strip()
        method = path_item.get("method", "").lower().strip()
        ...
        if request_schema is not None or request_example is not None:
        ...
        if response_schema is not None or response_example is not None:
            ...
            if response_example is not None:
                operation["responses"]["200"]["content"]["application/json"]["example"] = response_example
